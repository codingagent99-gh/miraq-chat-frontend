...
interface Props {
   ...
   * (e.g. "London White" -> { Colors: "LONDON White" }), plus any axis the
   * catalog itself settles because every variation shares one value. Values
   * match entries in `attributes[].options` exactly, so they can be compared
   * directly. Used to seed the initial selection so the user isn't asked to
   * re-pick something already decided.
   */
...
export function BulkVariantPromptCard({
  ...
  // The axes are NOT independent. A product may ship one colour in Polished
  // and Silky only, with no Honed variation — but every Finish was offered
  ...
  // braces alongside the per-option disabling below.
  const comboExists =
