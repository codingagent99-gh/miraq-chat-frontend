...
export function useHealthMonitor(
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      // /health answers without a license too, but only reports THIS
      // tenant's store health when one is supplied — see _health_loader_for_request
      // in server.py. Without it `components.store` comes back "unknown".
      const res = await fetch(`${apiUrl}/health`, {
        signal: controller.signal,
        headers: licenseId ? { "X-MiraQ-License-Id": licenseId } : undefined,
      });
      clearTimeout(t);
